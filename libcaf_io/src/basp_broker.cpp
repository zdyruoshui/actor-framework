  ...
  switch (ctx.state) {
    default: {
      ...
      if (!basp::valid(ctx.hdr)) {
        CAF_LOG_INFO("received invalid broker message " << ctx.hdr.operation);
        close(ctx.hdl);
        m_ctx.erase(ctx.hdl);
        return;
      }
...
void basp_broker::dispatch(const actor_addr& from, const actor_addr& to,
  ...
  CAF_LOG_DEBUG("routing message via " << to_string(route.node));
  if (route.invalid()) {
    parent().notify<hook::message_sending_failed>(from, to, mid, msg);
    CAF_LOG_ERROR("unable to dispatch message: no route to "
...
void basp_broker::init_client(connection_handle hdl,
  ...
  auto& ctx = m_ctx[hdl];
  ctx.hdl = hdl;
  init_handshake_as_client(ctx, std::move(data));
}
