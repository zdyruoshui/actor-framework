...
stream_based_peer::stream_based_peer(middleman* parent,
                                     const input_stream_ptr& in,
                                     const output_stream_ptr& out,
                                     node_id_ptr peer_ptr)
        : super(parent, out, in->read_handle(),
                out->write_handle(), peer_ptr)
        , m_in(in)
...
void stream_based_peer::io_failed(event_bitmask mask) {
    ...
    if (mask == event::read && has_node()) {
        // kill all proxies
        auto& children = parent()->get_namespace().proxies(node());
...
continue_reading_result stream_based_peer::continue_reading() {
    CPPA_LOG_TRACE("");
    for (;;) {
        ...
        switch (m_state) {
            case wait_for_process_info: {
                ...
                if (!parent()->register_peer(node(), this)) {
...
void stream_based_peer::monitor(const actor_addr&,
...
void stream_based_peer::kill_proxy(const actor_addr& sender,
                      const node_id_ptr& node,
                      actor_id aid,
                      std::uint32_t reason) {
...
void stream_based_peer::deliver(msg_hdr_cref hdr, any_tuple msg) {
    CPPA_LOG_TRACE("");
...
void stream_based_peer::link(const actor_addr& lhs, const actor_addr& rhs) {
...
void stream_based_peer::unlink(const actor_addr& lhs, const actor_addr& rhs) {
    CPPA_LOG_TRACE(CPPA_TARG(lhs, to_string) << ", "
                   << CPPA_TARG(rhs, to_string));
...
continue_writing_result stream_based_peer::continue_writing() {
    CPPA_LOG_TRACE("");
    ...
    if (result == continue_writing_result::done
            ...
            && !has_unwritten_data()) {
        if (parent()->get_namespace().count_proxies(node()) == 0) {
...
void stream_based_peer::add_type_if_needed(const std::string& tname) {
...
void stream_based_peer::enqueue_impl(msg_hdr_cref hdr, const any_tuple& msg) {
    CPPA_LOG_TRACE("");
...
void stream_based_peer::enqueue(msg_hdr_cref hdr, const any_tuple& msg) {
    enqueue_impl(hdr, msg);
    register_for_writing();
}

void stream_based_peer::dispose() {
